...
- 新库：在 `backend/` 目录执行 `alembic upgrade head`
- 已由 SQL 脚本初始化的旧库：执行 `python scripts/reconcile_alembic.py`

3. 执行本地 bootstrap

- 在 `backend/` 目录执行 `python scripts/bootstrap_local.py`
- 脚本会幂等确保本地演示教师账号存在，并尝试初始化 Milvus 必需集合
- 如果本地 Milvus 未启动或依赖环境异常，脚本会明确输出失败原因
