<!-- @Date: 2026-04-14 @Author: xisy @Discription: EduWeave 后端开发说明 -->

# EduWeave Backend

EduWeave 后端当前已完成阶段一输入链与阶段二知识结构化链路：认证、项目、教材、学情、教材解析、解析确认、知识抽取、知识版本管理、知识人工修正、任务中心和通用文件下载地址；教材解析与学情抽取已替换为真实 MinerU 接入，知识抽取通过 OpenAI 兼容接口接入结构化 LLM 与独立 Embedding 服务。
...
- 阶段二新增 OpenAI 兼容知识抽取配置：`LLM_API_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL`、`LLM_TIMEOUT_SECONDS`
