...
- 已稳定接口：
  ...
  - `/api/v1/agent/runs/{run_id}/events/list`
...
- MySQL 当前为 40 表 schema，包含教材语义块 `semantic_chunk`、一键生成 `generation_run`、教案课次恢复 `lesson_plan_generation_item`，以及智能助手会话、消息、运行、事件和工件 5 张表；Zilliz 云端集合已切换为 `semantic_chunk_vector` 与 `knowledge_point_vector`
- 智能助手模块内部已按 `runtime / tools / memory / writing` 分层：运行循环、工具注册与执行、上下文工件记忆、资源版本写入分别维护
- 当前自动化测试基线：`287 passed`
...
- 阶段二与阶段三共用 OpenAI 兼容结构化 LLM 配置：`LLM_API_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL`、`LLM_API_FORMAT`、`LLM_TIMEOUT_SECONDS`、`LLM_MAX_RETRIES`、`LLM_RETRY_BASE_SECONDS`、`LLM_STREAM_ERROR_DETAIL_MAX_CHARS`
...
- 教案生成阶段可通过 `LESSON_PLAN_MAX_CONCURRENCY` 控制并发数，默认 10，范围 1-10；系统会先串行生成第 1 课为 Packy 建立 prompt cache，再最多 10 并发生成剩余课次；课次级恢复可通过 `LESSON_PLAN_SESSION_MAX_RETRIES` 和 `LESSON_PLAN_SESSION_RETRY_BASE_SECONDS` 控制单课次 LLM 可恢复失败的重试次数与退避
...
- LLM 与教案课次恢复可由环境变量调整：`LLM_STREAM_ERROR_DETAIL_MAX_CHARS`（SSE 安全错误 detail 最大长度，默认 4096）、`LESSON_PLAN_SESSION_MAX_RETRIES`（单课次 LLM 可恢复失败重试次数，默认 2）、`LESSON_PLAN_SESSION_RETRY_BASE_SECONDS`（课次重试退避基数，默认 3）
...
- 日常开发不再推荐直接手工执行初始化 SQL 初始化新环境，统一走 Alembic
...
- 生成批次当前自动顺序编排课程大纲、多课次教案与初始覆盖率分析；多课时教案生成先写入 `lesson_plan_generation_item` 中间结果，全部课次成功后再批量转正式 `lesson_plan`，失败任务可通过 `POST /api/v1/tasks/{task_id}/retry` 按课次断点续跑；测评蓝图、单元测试试卷和 Raccoon PPT 课件由用户按需触发，并继续复用同一个 `generation_batch` 保留血缘，轻量 `generation_trace` 已随覆盖率报告写入，完整 `audit` 模块后续继续接入同一个批次根
...
- 异步任务具备崩溃恢复与失败重试能力：Celery beat 的 `system.reap_stale_tasks` 周期回收卡在 `processing` 的僵尸任务，可重试错误（LLM、MinerU、Raccoon 等外部依赖瞬时类）按 `task_record.retry_count`/`max_retry_count` 指数退避自动重排，业务校验类错误直接判失败；等待外部异步结果的停泊任务不会被误判回收；当前手动重试首版仅支持当前教师重试失败态 `lesson_plan_generate` 任务
