...
@Date: 2026-05-04
@Author: xisy
@Discription: 基于赛题材料运行整本教材到大纲教案的效果测试脚本
"""
...
os.environ["MINERU_POLL_TIMEOUT_SECONDS"] = os.environ.get("MINERU_POLL_TIMEOUT_SECONDS", "1800")
...
TEXTBOOK_NAME = "福建教育出版社英语五年级上册整本"
...
def _parse_textbook_with_mineru(
    ...
    *,
    ...
    textbook_version = TextbookVersion(
        ...
        auto_identify_json={"source": "education_competition_material", "scope": "full_textbook"},
        remark="效果测试使用整本教材进行 MinerU 解析。",
        uploaded_by=operator_user_id,
    )
    ...
    parse_version = ParseVersion(
        ...
        version_no=1,
        parse_mode=TEXTBOOK_PARSE_MODE,
        page_range_text=f"1-{page_count}",
        strategy_code="mineru_vlm_default",
...
def _write_report(
    ...
    *,
    ...
    textbook_page_count = len(PdfReader(str(TEXTBOOK_PATH)).pages)
    ...
    lines = [
        ...
        "",
        f"- 项目 ID：`{project_id}`",
        f"- 教材版本 ID：`{textbook_version_id}`，教材：`{TEXTBOOK_PATH.name}`，整本范围：`1-{textbook_page_count}`",
