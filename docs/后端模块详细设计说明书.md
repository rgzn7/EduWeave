...
flowchart LR
    ...
    pipeline -.批次编排.-> assessment
    pipeline -.批次编排.-> courseware
...
| `shared.vector` | Milvus 接入适配、集合管理、向量写入、删除、检索、健康检查 |
| `shared.utils` | 哈希计算、分页工具、时间工具、文本清洗、枚举与常量工具 |

`shared` 的关键规则如下：

- `shared.vector` 对接的是 Milvus 向量数据库组件，而不是自建向量引擎。
- `shared.vector` 在 P0 阶段即需提供可用实现，用于教材分块、知识点与证据召回。
...
`knowledge` 模块负责把解析结果转换成章节树、知识点矩阵、教学目标摘要与来源证据，是生成链路的基础数据源。

| 设计项 | 说明 |
| --- | --- |
| 模块定位 | 从已确认可用的解析版本抽取知识结构，并维护知识版本 |
| 核心输入 | 解析版本、知识点人工修正内容 |
| 核心输出 | 章节树、知识点矩阵、教学目标摘要、知识版本详情 |
...
- 教案版本与课程大纲版本保持一一对应的来源记录，但允许在同一课程大纲上生成多个教案版本。
...
`coverage` 模块对课程大纲、教案、课件和试卷进行知识点覆盖率和分布校验，是结果质控模块。
...
| 依赖模块 | `pipeline`、`knowledge`、`assessment`、`courseware`、`shared.queue` |

关键业务规则如下：

- 覆盖率分析只显式接收 `generation_batch_id`；所需知识版本、课程大纲、教案、课件和试卷结果统一从批次快照和批次结果中解析。
...
“课程大纲 -> 教案 -> 测评蓝图 -> 课件 -> 试卷 -> 覆盖率分析”这一整条链路，建议由 `pipeline` 模块以 `generation_batch` 作为统一编排根：

- `pipeline` 独占 `generation_batch` 的生命周期和批次级查询能力。
- `curriculum`、`lesson_plan`、`assessment`、`courseware`、`coverage` 五个模块共享同一生成批次标识。
- 同一批次内全部结果使用同一组输入版本。
