...
| 设计项 | 说明 |
| --- | --- |
| 模块定位 | 基于课程大纲生成教案版本，并支持课次级失败恢复 |
| 核心输入 | 课程大纲版本、教案风格参数 |
| 核心输出 | 教案结构化内容、课次中间结果、导出文档、版本信息 |
...
- 多课时教案生成使用 `lesson_plan_generation_item` 保存每个课次的中间结果，唯一键为 `(generation_batch_id, class_session_no)`；失败时不写入部分正式 `lesson_plan`，只保留可恢复的课次中间结果。
- 自动重试只重新生成 `pending`、`processing`、`failure` 或缺失的课次，已成功课次直接复用中间结果；全部课次成功后再批量转成正式 `lesson_plan` 并触发覆盖率任务。
- 课次终态失败时，任务失败 detail 只公开失败课次号、已处理课次数、并发上限、课次重试次数和安全错误摘要，不写出 prompt、traceback、API key 或原始长错误。
...
`task_center` 模块是统一任务视图层和任务恢复支撑层，不直接拥有某个业务对象；P0 当前提供任务查询、步骤进度、错误摘要、自动恢复和多课时教案失败任务手动重试。
...
- 任务状态必须与需求规格文档保持一致。
- 自动重试和僵尸任务回收由 `task_center.recovery` 根据任务类型注册表重新派发；用户主动重试首版开放 `POST /api/v1/tasks/{task_id}/retry`，仅允许当前教师重试 `failure` 状态的 `lesson_plan_generate` 任务。
- 手动重试会清空任务错误和结果、重置步骤进度、恢复关联 `generation_batch` 与 `generation_run` 状态、轮换 `execution_attempt_id`，并复用原 `payload_json` 重新派发。
